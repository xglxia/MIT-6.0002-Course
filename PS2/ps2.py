...
def load_map(map_filename):
    ...
    for line in data:
        ...
        src = Node(From)
        dest = Node(To)
        line_edge = WeightedEdge(src, dest, TotalDistance, DistanceOutdoors)
        if not graph.has_node(src):
            graph.add_node(src)
        if not graph.has_node(dest):
            graph.add_node(dest)
        graph.add_edge(line_edge)
...
#
# Answer: Minimize the total distance traveled while not exceeding the maximum distance outdoors.
#
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    for edge in digraph.get_edges_for_node(start):
        ...
        if node not in path[0]:
            ...
            new_path = get_best_path(digraph, node, end, updated_path, max_dist_outdoors, best_dist,
                  best_path)
            if new_path!= None:
